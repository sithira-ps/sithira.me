...
export default function BlogPage({ pageNumber = 1, category = 'all' }: { pageNumber: number, category: string }) {
  const sortedPosts = allPosts.sort((a, b) => new Date(b.date).getTime() - new Date(a.date).getTime())
  const filteredPost = category === 'all'
      ? sortedPosts
      : sortedPosts.filter((post) => post.tags?.includes(category));
